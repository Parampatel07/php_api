...
/*
     ...
     how to call : http://localhost/api/bookmychef/chef_change_password.php?chef_id=1&oldpassword=987987&newpassword=123123&confirmpassword=123123
     output :
     [{"error":"input is missing"}] 
     [{"error":"no"},{"success":"no"},{message:"Invalid change attempt"}]
     [{"error":"no"},{"success":"yes"},{message:"password changed successfully "}]
     input : chef_id,oldpassword,newpassword,confirmpassword
     */
...
if (
    ...
    if ($input['newpassword'] == $input['confirmpassword']) {
        //continue
        $sql = 'SELECT password from chef where id = ? ';
        $stat = $db->prepare($sql);
        $stat->setFetchMode(PDO::FETCH_ASSOC);
        $stat->bindparam(1, $input['chef_id']);
        $stat->execute();
        $row = $stat->fetch();
        if ($input['oldpassword'] == $row['password']) {
            // continue
            $sql = 'UPDATE chef set password = ? where id  = ? ';
            $stat = $db->prepare($sql);
            $stat->bindparam(1, $input['newpassword']);
            $stat->bindparam(2, $input['chef_id']);
            $stat->execute();
