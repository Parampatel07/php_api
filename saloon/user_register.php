...
/*
        ...
        how to call : http://localhost/api/sallon/user_login.php?email=ankit3385@gmail.com&password=123123
        output :
...
if (isset($input['email'], $input['password']) == false) {
    ...
    $sql = 'SELECT password,id from saloon_user where email = ? ';
    $stat = $db->prepare($sql);
    ...
    if ($count == 0) {
        ...
        if ($input['password'] == $row['password']) {
            ...
            array_push($response, ['userid' => $row['id']]);
