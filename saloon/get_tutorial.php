...
/*
          ...
          how to call : http://localhost/api/saloon/get_tutorial.php
          how to call : http://localhost/api/saloon/get_tutorial.php?tutorialid=1
...
$response = [];
$input = $_REQUEST;
if (isset($input['tutorialid']) == true) {
    $sql = 'SELECT * from saloon_tutoiral where id = ?';
    $stat = $db->prepare($sql);
    $stat->setFetchMode(PDO::FETCH_ASSOC);
    $stat->bindParam(1, $input['tutorialid']);
    $stat->execute();
    $table = $stat->fetchAll();
    // var_dump($table);
    ...
    $sql = 'SELECT * from saloon_tutoiral ';
    $stat = $db->prepare($sql);
    $stat->setFetchMode(PDO::FETCH_ASSOC);
    $stat->execute();
    $table = $stat->fetchAll();
