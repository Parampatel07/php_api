<?php
/*
    usage: used to login as customer 
    how to call : http://localhost/api/event_management/customer_login.php?email=param@gmail.com&password=987987
    output :
...
if (isset($input['email'], $input['password']) == false) {
    ...
    $sql = 'SELECT password,id from customer where email = ? ';
    $stat = $db->prepare($sql);
    ...
    if ($count == 0) {
        ...
        if ($input['password'] == $row['password']) {
            ...
            array_push($response, ['customerid' => $row['id']]);
